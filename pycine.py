...
# Favorite (sqlite)
# =============================
@app.post("/favorite/save/movies/")
async def save_favorite_movie(fav: schemas.FavModel ,db: Session = Depends(get_db)):
    user_id = 1 # o id do usuário fixo

    favorite_movie = models.Fav(idmovie=fav.idmovie, userid=fav.userid)
    db.add(favorite_movie)
    db.commit()
    db.close()
    return {"message": "Successfully saved favorite movie"}

#listar movies favoritos
# @app.get("/favorite/list/movies/")
# def list_favorite_movies(db: Session = Depends(get_db)):
#     user_id = 1 # o id do usuário fixo
#     favorite_movies = db.query(models.Fav).filter(models.Fav.userid == user_id).all()
#     db.close()

#     fav_movies = []
#     for movie in favorite_movies:
#         movie_details = RequestApi.get_movie(movie.idmovie)
#         fav_movies.append({
